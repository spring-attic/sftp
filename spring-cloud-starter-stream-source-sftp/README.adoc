...
$$aws.s3.access-key$$:: $$The S3 access key.$$ *($$String$$, default: `$$<none>$$`)*
$$aws.s3.bucket$$:: $$The S3 Bucket name.$$ *($$String$$, default: `$$<none>$$`)*
$$aws.s3.create-bucket$$:: $$Flag to create the bucket if it does not exist.$$ *($$Boolean$$, default: `$$true$$`)*
$$aws.s3.endpoint$$:: $$The S3 endpoint.$$ *($$URI$$, default: `$$<none>$$`)*
$$aws.s3.region$$:: $$The S3 Region (defaults to us-east-1, ignored by minio).$$ *($$Regions$$, default: `$$<none>$$`, possible values: `GovCloud`,`US_EAST_1`,`US_EAST_2`,`US_WEST_1`,`US_WEST_2`,`EU_WEST_1`,`EU_WEST_2`,`EU_WEST_3`,`EU_CENTRAL_1`,`AP_SOUTH_1`,`AP_SOUTHEAST_1`,`AP_SOUTHEAST_2`,`AP_NORTHEAST_1`,`AP_NORTHEAST_2`,`SA_EAST_1`,`CN_NORTH_1`,`CN_NORTHWEST_1`,`CA_CENTRAL_1`)*
$$aws.s3.secret-key$$:: $$The S3 secret key.$$ *($$String$$, default: `$$<none>$$`)*
...
$$sftp.transfer-to$$:: $$The target storage to use for the file transfer (LOCAL, CF_VOLUME, S3) default is `LOCAL`$$ *($$TransferType$$, default: `$$<none>$$`, possible values: `LOCAL`,`CF_VOLUME`,`S3`)*
