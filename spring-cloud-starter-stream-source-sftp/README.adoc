...
the `task-launcher-output` property to `DATAFLOW` or `STANDALONE`. It is recommended
...
$$sftp.task.task-launch-request$$:: $$Set to output a task launch request (STANDALONE, DATAFLOW, NONE) default is `NONE`.$$ *($$TaskLaunchRequestType$$, default: `$$<none>$$`, possible values: `NONE`,`DATAFLOW`,`STANDALONE`)*
